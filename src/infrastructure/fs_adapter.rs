...
impl FsAdapter {
    async fn apply_directory(&self, dir: &crate::domain::resource::DirectoryResource) -> Result<()> {
        if dir.ensure == Ensure::Present {
            ...
            self.apply_metadata(&dir.path, &dir.owner, &dir.group, &dir.mode).await?;
    ...
    async fn apply_metadata(&self, path: &std::path::Path, owner: &Option<String>, group: &Option<String>, mode: &Option<String>) -> Result<()> {
        // Mode
        if let Some(mode_str) = mode {
            let mode_val = u32::from_str_radix(mode_str, 8)
                .map_err(|e| DomainError::Internal(format!("Invalid mode octal string '{}': {}", mode_str, e)))?;
        ...
        let uid = if let Some(owner_name) = owner {
            let user = User::from_name(owner_name)
                .map_err(|e| DomainError::Internal(format!("Failed to resolve user '{}': {}", owner_name, e)))?
        ...
        let gid = if let Some(group_name) = group {
            let grp = Group::from_name(group_name)
                .map_err(|e| DomainError::Internal(format!("Failed to resolve group '{}': {}", group_name, e)))?
                .ok_or_else(|| DomainError::Internal(format!("Group '{}' not found", group_name)))?;
            Some(grp.gid)
        } else {
            None
        };
    ...
    async fn apply_file(&self, file: &FileResource) -> Result<()> {
        match file.ensure {
            Ensure::Present => {
                ...
                self.apply_metadata(&file.path, &file.owner, &file.group, &file.mode).await?;
