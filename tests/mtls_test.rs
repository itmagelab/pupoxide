...
use pupoxide::interface::server::{start_master, MasterState};
use std::fs;
...
async fn test_mtls_full_workflow() -> Result<()> {
    ...
    assert!(check_res.is_err(), "Should timeout because it's not approved yet");
    ...
    agent.check_bootstrap_status(5).await.expect("Checking bootstrap status failed after approval");
    ...
    state.bootstrap_manager.approve_request("unauthorized-agent").await?;
    ...
    let client_pem = fs::read_to_string(certs_dir.join("agents").join("unauthorized-agent").join("agent.pem"))?;
    let client_key = fs::read_to_string(certs_dir.join("agents").join("unauthorized-agent").join("agent.key"))?;
    let ca_pem = fs::read_to_string(certs_dir.join("agents").join("unauthorized-agent").join("ca.pem"))?;
    ...
    let resp = client.post(&spoof_url).json(&Facts::default()).send().await?;
    assert_eq!(resp.status(), reqwest::StatusCode::FORBIDDEN, "Server should reject spoofed catalog requests!");
